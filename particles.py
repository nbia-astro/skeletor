...
class Particles (numpy.ndarray):
    ...
    def __new__ (cls, x, y, vx, vy, npmax):
        ...
        assert npmax >= np
        if npmax < int (5/4*np):
            warn ("Particle array is probably not large enough" \
                    + " (np={}, npmax={})".format (np, npmax))
    ...
    def push (self, grid, dt):
        ...
        ihole, ek = cppgpush2l (self, grid.edges, self.np, grid.noff,
                dt, grid.nx, grid.ny, self.idimp, self.npmax, grid.mx,
                grid.nypmx, self.idps, self.ntmax, self.ipbc)
        ...
        kstrt = comm.rank + 1
        nvp = comm.size
        self.np, info = cppmove2 (self, grid.edges, self.np, ihole,
                grid.ny, kstrt, nvp, self.idimp, self.npmax, self.idps,
                self.nbmax, self.ntmax)
