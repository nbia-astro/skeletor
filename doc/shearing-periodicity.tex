...
\labelcref{eq:continuity} and the momentum equation
\labelcref{eq:momentum-cold-neutral} reduce to
\begin{equation}
...
\end{equation}
The momentum equation \labelcref{eq:momentum-1D} in Lagrangian coordinates is
thus given by
\begin{equation}
...
\end{equation}
where $\rho_0$ is independent of $\tau$. \Cref{eq:initial-x} implies that
...
\labelcref{eq:ux-prime,eq:momentum-lagrangian,eq:dx/dtau,eq:initial-x,eq:rho}
...
\labelcref{eq:momentum-lagrangian} are
\begin{align}
  \label{eq:dux/dtau}
...
\subsubsection{Non-linear evolution of sheared disturbances: No rotation}

In this section we consider the solution to \cref{eq:dux/dtau,eq:duy/dtau} in
the special case where there is no rotation, i.e.\ $\Omega=0$. We find
\begin{align}
  \pader{\tilde{u}_x}{\tau} &= S\tilde{u}_y \\
  \pader{\tilde{u}_y}{\tau} &= 0
\end{align}
for which one simple solution is
\begin{align}
  \tilde{u}_x &= U(a) \\
  \tilde{u}_y &= 0.
\end{align}
Here, $U(a)$ is a function of the Lagrangian coordinate, $a$. It then follows
from \cref{eq:ux-prime} that $\tilde{u}_{x'} = U(a)$ and using
\cref{eq:dx/dtau} we find that
\begin{equation}
  \label{eq:xprim-burgers}
  x' = U(a)\,\tau + a .
\end{equation}
The analytical solution for the density is found according to \cref{eq:rho}
\begin{equation}
  \label{eq:rho-sheared-burgers}
  \rho(a, t) = \rho_0 \left(1 + \pder{U}{a} \tau\right)^{-1}.
\end{equation}
Note that the 2D solution $\rho(x, y, t)$ can be found from the solution $\rho
(a, t)$ by using the definition $x'= x + Sty$ in \cref{eq:xprim-burgers} and
solving for $a$ using the Newton-Raphson method. If we choose $U(a) = A \sin{k
a}$ the solution for the density in the primed coordinate will be the same as
for the Eulerian coordinate in the standard 1D Burgers' equation.
