...
\begin{equation}
  ...
  \left(\pder{}{t} - Sy\pder{}{x}\right)f + \tilde{\vec{v}}\cdot\nabla f
  + (\vec{a} - 2\vec{\Omega}\times\tvec{v} + S\tilde{v}_y\ex - \nu^2 z\ez)
  \cdot\pder{f}{\tvec{v}} = 0.
\end{equation}
While this equation appears more complicated than \cref{eq:vlasov}, it has the
distinct advantage that it depends explicitly on $y$ only through the
differential operator $\pader{}{t}-Sy\,\pader{}{x}$. This explicit
$y$-dependence can be eliminated (in favor of an explicit $t$-dependence)
through a transformation to shearing coordinates, see
...
\begin{equation}
  ...
  \left(\pder{}{t} - Sy\pder{}{x}\right)\vec{B} + SB_y\ex
  + \nabla\times\tvec{E} = 0.
\end{equation}
This form of Faraday's law makes explicit that the magnetic field is advected
and stretched by the uniform shear flow. Note that the differential operator
$\pader{}{t}-Sy\,\pader{}{x}$ appearing here is the same as in
\cref{eq:vlasov-relative}.
